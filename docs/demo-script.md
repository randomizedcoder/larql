...
A signal strength line comes first — `clean (N edges, max gate 35.2)` — telling you this entity has strong, well-separated features. Then three stages come back, matching the residual stream architecture:
...
You can see this directly. Try an abstract concept:

```sql
DESCRIBE "however";
```

Signal strength: `diffuse (12 edges, max gate 7.3)`. Compare that with France's `clean (28 edges, max gate 35.2)`. The features that fire for "however" are discourse and transition markers — real patterns, but spread thin across many contexts. DESCRIBE faithfully shows this diffuseness rather than hiding it. Entities have clean edges. Abstract tokens have diffuse ones. The signal strength line tells you which you're looking at.
...
Verify it against the original model:

```sql
INFER "The capital of France is" TOP 5 COMPARE;
```

Walk FFN: Paris (80.47%). Dense FFN: Paris (80.01%). Same answer, same probability. The walk produces identical predictions to the dense forward pass — zero divergence at any layer boundary across the full test set. COMPARE runs both backends and shows you the numbers side by side.
...
One statement. The INSERT pipeline runs a forward pass on the canonical prompt "The capital of Atlantis is", captures the model's residual at L26 — the vector the FFN actually sees at the knowledge layer — and installs a new FFN feature at a free slot at that layer: the captured residual becomes the feature's gate row, a norm-matched projection of the target token embedding becomes its down column, and the meta carries the `Atlantis → Poseidon` relation label. A refine pass orthogonalises the new gate against decoy residuals so the install doesn't hijack unrelated prompts. A balance pass auto-tunes the down-column magnitude until the target lands in a clean probability band on the canonical prompt. All three steps happen automatically inside the one statement.
...
Poseidon. 99.99%. During inference, the forward pass runs as normal — attention routes the prompt, the FFN at L26 computes gate scores for all features, and our new feature's gate dot-product with the current residual is near-maximal (because we installed the gate as a copy of that same residual). Its activation fires, its down column writes `embed("Poseidon")` into the residual stream, and by the final layer lm_head returns "Pose" at the top of the logits. One new feature, one forward pass, one answer.
...
Paris. Preserved. The France query produces a different residual at L26 — different entity means different residual direction. Our new feature's gate has near-zero dot-product with the France residual (it's orthogonal to the refine-pass decoys), so the feature doesn't fire. The model answers from its own trained features, untouched.

The FFN is the KNN store. The INSERT didn't add a parallel retrieval layer — it added one feature to the graph that lives in the gate and down matrices. Each fact is an independent feature; no orthogonality bookkeeping because the refine pass handles it per-install.
...
The insert is visible — Poseidon appears at L26 as a compiled edge.
...
This bakes the patched features into a standalone vindex — the installed gate rows are written into `gate_vectors.bin` and the installed down columns into `down_weights.bin` at the feature slots we allocated. The base files aren't modified; a new copy is produced with the overlay applied. The compiled vindex loads in any session and the facts look identical to features the model was trained with — because structurally they are.

When multiple patches touch the same entities, conflict resolution is explicit:

```sql
COMPILE CURRENT INTO VINDEX "/tmp/merged.vindex" ON CONFLICT HIGHEST_CONFIDENCE;
```

Three strategies: `LAST_WINS`, `HIGHEST_CONFIDENCE`, or `FAIL` (abort if collisions exist). Because each INSERT allocates its own slot, entries about the same entity simply coexist as separate features; conflict resolution decides what to do when two patches try to write the same slot.
...
Poseidon. Still there. The compiled-in features survived the round-trip because they're weights now — not an overlay.
...
That's the full loop: **query the graph -> insert new knowledge -> verify no regression -> compile to permanent vindex**. Same query language for reading and writing. The model is a database you can edit.
...
|---|---|
| `DESCRIBE "entity"` | The node view — everything the model knows, with signal strength |
...
| `INFER "prompt" COMPARE` | Walk vs dense side-by-side — verify round-trip fidelity |
| `INSERT INTO EDGES` | Allocate a free FFN slot and install a new compiled feature |
| `COMPILE INTO VINDEX` | Bake overlay features into a standalone vindex |

One engine. One query language. 348,160 edges in the base graph, more added by growing the FFN one feature at a time. The model is a database.
